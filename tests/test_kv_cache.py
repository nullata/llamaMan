...
class BuildLlamaCmdKvCacheTests(unittest.TestCase):
    ...
    def test_flash_attn_off_omits_flag(self):
        cmd = build_llama_cmd("/models/m.gguf", 8080, {})
        self.assertNotIn("--flash-attn", cmd)

    def test_flash_attn_on_emits_flag(self):
...
class LaunchInstanceKvCacheFieldsTests(unittest.TestCase):
    ...
    normalization (bool coercion for flash_attn, lowercase+trim for the cache
    types), so the running instance's config is authoritative for later reads
    - restart, cluster snapshot, live preset merge."""
    ...
    def test_launch_instance_stores_kv_fields(
        ...
        self.assertTrue(inst["config"]["flash_attn"])
    ...
    def test_launch_instance_defaults_are_zero_values(
        ...
        self.assertIsNone(err)
        self.assertFalse(inst["config"]["flash_attn"])
...
class InstancesCreateRouteKvCacheTests(unittest.TestCase):
    ...
    def test_create_route_defaults_omitted_kv_fields(self, launch_mock):
        ...
        kwargs = launch_mock.call_args.kwargs
        self.assertFalse(kwargs["flash_attn"])
        self.assertEqual(kwargs["cache_type_k"], "")
        self.assertEqual(kwargs["cache_type_v"], "")

    @patch("api.instances.launch_instance")
    def test_create_route_coerces_flash_attn_to_bool(self, launch_mock):
        # A JS client always sends a real boolean, but other callers may send
        # 1 / "true" / "on". The route uses bool() at the boundary so
        # downstream code sees an actual bool.
        launch_mock.return_value = ({"id": "inst-1"}, None)
        with patch("api.instances._public_instance", side_effect=lambda inst: inst):
            self.client.post(
                "/api/instances",
...
class PresetKvCacheTests(unittest.TestCase):
    ...
    def test_preset_save_persists_kv_fields(self):
        ...
        self.assertTrue(saved_preset["flash_attn"])
    ...
    def test_preset_save_defaults_are_off_and_empty(self):
        ...
        _, saved_preset = storage.save_preset.call_args.args
        self.assertFalse(saved_preset["flash_attn"])
...
class MergePresetIntoConfigKvCacheTests(unittest.TestCase):
    ...
    def test_merge_preset_overlays_kv_fields(self):
        ...
        preset = {"flash_attn": True, "cache_type_k": "q8_0", "cache_type_v": "q4_0"}
        ...
        self.assertTrue(merged["flash_attn"])
    ...
    def test_merge_preset_overrides_base_kv_fields(self):
        ...
        base_config = {"flash_attn": False, "cache_type_k": "", "cache_type_v": ""}
        preset = {"flash_attn": True, "cache_type_k": "q8_0", "cache_type_v": "q8_0"}
    ...
    def test_merge_preset_leaves_base_when_preset_omits_kv_fields(self):
        ...
        base_config = {"flash_attn": True, "cache_type_k": "q8_0", "cache_type_v": "q8_0"}
