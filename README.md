...
- **Image & PDF Input** - loads a vision model's multimodal projector (`--mmproj`) so image and PDF inputs work. Inbound PDFs are rasterized page-by-page to images before the vision model sees them (llama-server has no native PDF support); optional text-layer shortcut inlines born-digital PDFs (Word/LaTeX/browser print) as plain text with no rasterization pass. Works on both OpenAI `image_url` / `file` content blocks and Ollama's native `images[]` array. Per-request DPI and max-pages caps plus a process-wide raster semaphore keep bursty PDF traffic from spiking the host
...
| **Image & PDF Input** | off | Master toggle for the section. When on, loads a multimodal projector (`--mmproj`) so a vision model accepts image inputs, and unlocks the per-instance PDF settings below. See [Image & PDF Input](#image--pdf-input). |
| **MMPROJ Model** | _(none)_ | Path to the multimodal projector GGUF (`--mmproj`). Required when Image & PDF Input is on. Pick a downloaded GGUF from the list or type a path under the models directory. |
| **Accept PDF uploads** | off | Also accept PDFs on this instance. Each page is rasterized to a PNG block before forwarding to llama-server (which has no PDF support). Requires Image & PDF Input to be on. |
| **Try text layer first** | off | For born-digital PDFs (Word/LaTeX/browser print), skip rasterization and inline the embedded text layer as one plain text block. Falls through to rasterization on scans / sparse text (uses a 50-chars/page heuristic). Off by default: rasterization preserves layout information for the vision model, and predictable behavior on every PDF beats a silent shortcut. |
| **PDF DPI** | `200` | Rasterization resolution (`72`–`600`). 200 is a solid default; 300 for dense text. Higher wastes CPU and RAM without extra quality — vision models resize internally. |
| **Max PDF Pages** | `20` | Per-request cap on rasterized pages (`1`–`200`). A PDF over this size returns HTTP 400 rather than being silently truncated. |
...
Enable **Image & PDF Input** on an instance to load a vision model's multimodal projector (`--mmproj`) alongside its main GGUF, so the model can consume `image_url` content blocks in OpenAI-shaped requests or the Ollama-native `images[]` array. The **MMPROJ Model** field takes the projector GGUF that ships alongside the main weights (typical filename: `mmproj-<model>-<precision>.gguf`).

Turning on **Accept PDF uploads** additionally lets llamaMan handle PDFs in the same content blocks by rasterizing each page to a PNG **before** forwarding to llama-server — llama-server itself has no PDF support and would reject the payload otherwise. Pages are appended in order as `image_url` blocks in the same message. Both entry points are recognized:

- **OpenAI**: `{"type":"image_url","image_url":{"url":"data:application/pdf;base64,..."}}` or the newer file shape `{"type":"file","file":{"file_data":"data:application/pdf;base64,..."}}`
- **Ollama**: base64 PDF bytes mixed into a message's `images` array — llamaMan sniffs the `%PDF-` header, splits PDFs out, and expands them; ordinary images in the same array keep their existing lifting path

**Text-layer shortcut** (off by default): with **Try text layer first** enabled, llamaMan runs `pypdf.extract_text()` before rasterizing. If the PDF is born-digital (Word/LaTeX/browser print) and its embedded text is substantive (≥ ~50 characters per page), llamaMan inlines that text as a single `{"type":"text","text":"..."}` block and skips the raster/vision path entirely — no image, no vision-model tokens. Sparse or empty text (the scanned-PDF case) falls through to rasterization automatically. Left off by default because rasterization preserves layout information for the vision model, and predictable behavior on every PDF is worth more than a silent shortcut whose token cost swings order-of-magnitude between requests.

**Per-request caps**: **PDF DPI** sets the rasterization resolution (72–600, default 200); **Max PDF Pages** caps how many pages one request may include (default 20). A PDF over the page cap returns HTTP 400 rather than being silently truncated.

**Rasterization concurrency**: PDF rendering is CPU- and RAM-heavy (poppler subprocess plus in-memory pixmaps) and runs in the gunicorn worker thread *before* the request reaches any per-instance concurrency gate. A process-wide semaphore caps how many pages can be rendering at once so a burst of PDF traffic can't spike host CPU/RAM beyond available resources; tune via `LLAMAMAN_PDF_MAX_CONCURRENT` (default 4).

**Error surfacing**: a malformed PDF or one over the page cap returns HTTP 400 with a `PDF input error: ...` message. Missing poppler at runtime returns HTTP 500 (`PDF support unavailable`) — the module logs a warning at startup so you notice before traffic arrives. If **Image & PDF Input** is off on the target instance, PDF blocks pass through unchanged and llama-server surfaces its own error — the correct "you didn't enable this" behavior.
...
- **Sleeping instances still count toward the limit** (their slot claim persists across the idle-timeout pause), **but waking a sleeping instance for its own model is never blocked by the cap.** That slot was already claimed at launch; sleep is a pause, not a release. The cap check applies only when a new instance would need to be created - so an API request for a *different* model, when the cap is full of admin-launched sleepers, still returns 503 unless the "Allow API to evict admin-launched" toggle is on.
...
| `LLAMAMAN_PDF_MAX_CONCURRENT` | `4` | Max concurrent PDF rasterizations across the whole process. Independent of per-instance concurrency gates. Each raster spawns a poppler subprocess and can spike a few hundred MB of transient RAM at high DPI, so this budget keeps a PDF burst from overwhelming the host. See [Image & PDF Input](#image--pdf-input). |
